...
const WorkerReputationSystem = ({ 
  ...
  const reputationApi = {
    async getWorkerReputation(workerId) {
      ...
      return {
        ...
        // Category expertise
        categoryExpertise: [
          { category: 'Carpentry', rating: 4.9, jobs: 45, specialization: 'Custom Furniture' },
          { category: 'Home Renovation', rating: 4.8, jobs: 32, specialization: 'Kitchen Remodeling' },
          { category: 'Repairs', rating: 4.7, jobs: 28, specialization: 'Emergency Fixes' },
          { category: 'Installation', rating: 4.6, jobs: 18, specialization: 'Cabinet Installation' }
        ],
  ...
  const getTrendDisplay = (trend, change = 0) => {
    if (trend === 'up') {
      ...
      return { icon: TrendingDownIcon, color: '#F44336', text: `-${Math.abs(change)}` };
    }
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <Box sx={{ flex: 1 }}>
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 0.5 }}>
              <Rating value={reputationData.averageRating} readOnly size="small" />
            ...
            <Stack direction="row" spacing={1}>
              {reputationData.trustMetrics.verificationStatus === 'fully_verified' && (
              ...
              <CardContent>
                ...
                <Box sx={{ 
                  ...
                  <Box sx={{ 
                    ...
                    <Typography variant="h3" sx={{ color: getScoreColor(reputationData.overallScore), fontWeight: 800 }}>
                      {reputationData.overallScore}
                    </Typography>
                    <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                      /100
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ 
            ...
            <CardContent>
              ...
              <Stack spacing={2}>
                {Object.entries(reputationData.ratings).map(([category, rating]) => (
                  <Box key={category}>
                        ...
                        >
                          <Tooltip title={achievement.description}>
                            <Paper sx={{ 
                              ...
                              >
                                <IconComponent sx={{ fontSize: 32, color: achievement.color, mb: 1 }} />
                              </Badge>
              ...
              <CardContent>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
                    ...
                    <Stack spacing={1.5}>
                      {Object.entries(reputationData.performanceTrends).map(([period, data]) => {
                        const trendDisplay = getTrendDisplay(data.trend, data.change);
                            ...
                            <Stack direction="row" alignItems="center" spacing={1}>
                              ...
                              {TrendIcon && (
                                <Stack direction="row" alignItems="center" spacing={0.5}>
                                  <TrendIcon sx={{ fontSize: 16, color: trendDisplay.color }} />
                                  <Typography variant="caption" sx={{ color: trendDisplay.color }}>
                                    {trendDisplay.text}
                                  </Typography>
                                </Stack>
                              )}
