...
const MilestoneTracker = ({
  ...
  const sampleMilestones = useMemo(() => [
    {
      ...
      description: 'Complete assessment of plumbing requirements and create detailed work plan',
      status: 'completed',
      priority: 'high',
      ...
      progress: 100,
      deliverables: [
        { id: '1a', name: 'Site assessment report', status: 'completed', file: '/docs/assessment.pdf' },
        { id: '1b', name: 'Work timeline', status: 'completed', file: '/docs/timeline.pdf' },
        { id: '1c', name: 'Material list', status: 'completed', file: '/docs/materials.pdf' }
      ],
      assignedTo: 'Kwame Asante',
      location: 'East Legon, Accra',
      notes: 'Site assessment completed successfully. All necessary permits obtained.',
      photos: ['/photos/site1.jpg', '/photos/site2.jpg']
    },
    {
      id: '2',
      title: 'Material Procurement',
      description: 'Purchase all required materials including pipes, fittings, and fixtures',
      status: 'in_progress',
      priority: 'medium',
      ...
      progress: 60,
      deliverables: [
        { id: '2a', name: 'Material receipts', status: 'completed', file: '/docs/receipts.pdf' },
    ...
    },
    {
      ...
      description: 'Install new water supply and drainage pipes throughout the building',
      status: 'pending',
      priority: 'high',
    ...
    },
    {
      ...
      description: 'Comprehensive testing of all plumbing systems and project handover',
      status: 'pending',
      priority: 'critical',
  ...
  const financialSummary = useMemo(() => {
    ...
    const paidAmount = milestones.reduce((sum, m) => sum + (m.paidAmount || 0), 0);
    ...
    return {
      ...
      percentagePaid: totalAmount > 0 ? Math.round((paidAmount / totalAmount) * 100) : 0
    };
  }, [milestones]);
  ...
  const updateMilestoneStatus = useCallback(async (milestoneId, newStatus) => {
    try {
  ...
  const requestPayment = useCallback(async (milestoneId) => {
    try {
      if (onPaymentRequest) {
  ...
  const renderMilestoneCard = useCallback((milestone) => {
    ...
    return (
      ...
      >
        <Card
          ...
          sx={{
            mb: 2,
            border: status === 'overdue' ? '2px solid #EF5350' : '1px solid rgba(255,215,0,0.2)',
        ...
        >
          ...
          <CardContent sx={{ pb: 1 }}>
              ...
              <Stack direction="row" spacing={1}>
                {isWorker && status === 'in_progress' && (
                  <Button
                    ...
                    onClick={() => updateMilestoneStatus(milestone.id, 'review')}
                ...
                {status === 'completed' && milestone.paidAmount < milestone.amount && isWorker && (
                  <Button
                    size="small"
                    variant="contained"
            ...
            <Collapse in={isExpanded}>
              <Box sx={{ mt: 3, pt: 2, borderTop: '1px solid rgba(255,215,0,0.2)' }}>
                {/* Deliverables */}
                        ...
                        <ListItem key={deliverable.id} sx={{ py: 0.5 }}>
                          <ListItemIcon>
                            {deliverable.status === 'completed' ? (
                              ...
                              <StartIcon sx={{ color: '#42A5F5', fontSize: 20 }} />
                            ) : (
                              <PendingIcon sx={{ color: '#FFA726', fontSize: 20 }} />
                            )}
                          </ListItemIcon>
                          <ListItemText
  ...
  }, [expandedMilestone, getMilestoneStatus, isWorker, isContractOwner, updateMilestoneStatus, requestPayment]);
    ...
    <Box>
      ...
      <Stack direction="row" spacing={1} sx={{ mb: 3, overflowX: 'auto', pb: 1 }}>
        <Chip
          ...
          sx={{
            backgroundColor: filterStatus === 'all' ? '#FFD700' : 'rgba(255,255,255,0.1)',
            color: filterStatus === 'all' ? '#000' : '#fff',
            fontWeight: 600
          }}
        />
        {Object.entries(milestoneStatuses).map(([status, info]) => {
          const count = milestones.filter(m => getMilestoneStatus(m) === status).length;
          return (
            <Chip
              key={status}
              ...
              sx={{
                backgroundColor: filterStatus === status ? info.color : 'rgba(255,255,255,0.1)',
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <Stack direction="row" spacing={2}>
              ...
              <TextField
                ...
                onChange={(e) => setNewMilestone(prev => ({ ...prev, amount: parseFloat(e.target.value) || '' }))}
                placeholder="0"
              />
            </Stack>
