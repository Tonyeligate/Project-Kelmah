...
export const validateColors = (element, expectedColors) => {
  ...
  if (element.color && !expectedColors.includes(element.color)) {
    errors.push(`Invalid color: ${element.color}. Expected one of: ${expectedColors.join(', ')}`);
  }
  
  if (element.bgcolor && !expectedColors.includes(element.bgcolor)) {
    errors.push(`Invalid background color: ${element.bgcolor}. Expected one of: ${expectedColors.join(', ')}`);
  }
...
export const validateTypography = (element, expectedVariants) => {
  ...
  if (element.variant && !expectedVariants.includes(element.variant)) {
    errors.push(`Invalid typography variant: ${element.variant}. Expected one of: ${expectedVariants.join(', ')}`);
  }
  
  if (element.fontSize && typeof element.fontSize === 'string' && !element.fontSize.includes('rem')) {
  ...
  Object.keys(element).forEach(key => {
    if (key.includes('padding') || key.includes('margin') || key.includes('gap')) {
      ...
      if (typeof value === 'number' && !expectedSpacing.includes(value)) {
        errors.push(`Invalid spacing value: ${key}=${value}. Expected one of: ${expectedSpacing.join(', ')}`);
      }
    }
  });
...
export const validateBreakpoints = (element) => {
  ...
  Object.keys(element).forEach(key => {
    if (validBreakpoints.includes(key)) {
      // Valid breakpoint
    } else if (key.includes('xs') || key.includes('sm') || key.includes('md') || key.includes('lg') || key.includes('xl')) {
...
export const checkThemeConsistency = (components) => {
  ...
  components.forEach((component, index) => {
    ...
    const colorErrors = validateColors(component, [
      '#D4AF37', '#FFD700', '#B8941F', // Gold variations
      '#1a1a1a', '#2d2d2d', '#0a0a0a', // Black variations
      '#ffffff', 'rgba(255,255,255,0.7)', 'rgba(255,255,255,0.5)', // White variations
      '#4CAF50', '#FF9800', '#F44336', '#2196F3', // Status colors
    ]);
    componentErrors.push(...colorErrors);
    
    // Check typography
    const typographyErrors = validateTypography(component, [
      'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'body1', 'body2', 'caption', 'button'
    ]);
...
export const generateThemeReport = (components) => {
  ...
  const complianceRate = ((totalComponents - componentsWithErrors) / totalComponents) * 100;
