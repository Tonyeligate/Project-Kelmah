...
const ProfessionalMapPage = () => {
  ...
  // Map state
  const [mapCenter, setMapCenter] = useState([5.6037, -0.1870]); // Accra, Ghana
  ...
  useEffect(() => {
    const initializeMap = async () => {
      try {
        ...
        await loadMapData(searchType, { latitude: 5.6037, longitude: -0.1870 });
      } finally {
          ...
          <Box sx={{ 
            ...
            >
              ...
              <ToggleButton value="workers">
                <WorkerIcon sx={{ mr: 1 }} />
                Workers ({searchResults.filter(r => r.type === 'worker').length})
              </ToggleButton>
            </ToggleButtonGroup>
          ...
          <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            {vocationalCategories.slice(0, 6).map((category) => (
              <Chip
                ...
                color={currentFilters.categories.includes(category) ? 'secondary' : 'default'}
                sx={{
            ...
            >
              <Paper sx={{
                ...
                <Typography variant="body2" sx={{ 
                  ...
                  {selectedLocation && ` near ${selectedLocation.city || 'selected area'}`}
                </Typography>
              </Paper>
            </motion.div>
          )}
          ...
          {loading && !initializing && (
            ...
            >
              <Paper sx={{
                ...
                <CircularProgress sx={{ color: theme.palette.secondary.main, mb: 2 }} />
