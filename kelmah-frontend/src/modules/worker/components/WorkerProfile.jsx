...
function WorkerProfile() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const [skillsRes, portfolioRes, certsRes, reviewsRes, historyRes] =
        await Promise.all([
      ...
      <Box sx={{ p: 3 }}>
        ...
        >
          <Skeleton
            ...
            width={150}
            height={150}
            sx={{ border: `4px solid ${theme.palette.background.paper}` }}
          />
  ...
  if (error || !profile) {
    return (
      <Alert severity="error" sx={{ m: 3 }}>
        {error || 'Could not load worker profile.'}
      </Alert>
    );
  }
  ...
  return (
    <Box
      sx={{ p: { xs: 1, sm: 2, md: 3 }, background: theme.palette.grey[100] }}
    >
      {/* Profile Header Card */}
      <Card
        elevation={3}
        ...
        >
          ...
          <Box sx={{ textAlign: 'center', p: 3 }}>
            ...
            >
              <Chip
                icon={<WorkIcon />}
                label={`${profile.experience_years || 0} years experience`}
                variant="outlined"
              />
              <Chip
                icon={<MoneyIcon />}
                label={`$${profile.hourly_rate || 0}/hour`}
                variant="outlined"
              />
              <Chip
                icon={<LocationIcon />}
