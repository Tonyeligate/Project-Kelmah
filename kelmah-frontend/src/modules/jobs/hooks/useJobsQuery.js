...
const sanitizeParams = (
  rawParams = EMPTY_QUERY_PARAMS,
  defaults = EMPTY_QUERY_PARAMS,
) => {
  const merged = {
    ...defaults,
    ...rawParams,
    ...
    onMutate: async (variables) => {
      ...
      const snapshots = [
        [
          ...
          queryClient.getQueryData(jobKeys.detail(jobId)),
        ],
      ...
      if (detailQuery && typeof detailQuery === 'object') {
        queryClient.setQueryData(
          jobKeys.detail(jobId),
          markJobAsApplied(detailQuery),
        );
      }

      queryClient
        .getQueriesData({ queryKey: jobKeys.list(), exact: false })
        .forEach(([key, data]) => {
    ...
    onMutate: async (variables) => {
      ...
      if (variables?.job) {
        snapshot.forEach(([key, data]) => {
          queryClient.setQueryData(
            key,
            updateSavedJobsCache(data, variables.job, 'add'),
          );
        });
      }
