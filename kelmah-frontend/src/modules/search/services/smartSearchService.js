...
const smartSearchService = {
  ...
  getSmartJobRecommendations: async (userId, options = {}) => {
    try {
      ...
      return {
        data: {
          jobs: generateMockJobRecommendations(userId, options),
          insights: {
            aiScore: 0.85,
            matchingFactors: ['Skills match', 'Location preference', 'Experience level'],
            improvementTips: ['Add more skills to your profile', 'Update your location preferences']
          },
          savedJobIds: []
        }
      };
...
const generateMockJobResults = (searchParams) => {
  ...
  return generateMockJobRecommendations('mock_user', { limit: 10 });
};
