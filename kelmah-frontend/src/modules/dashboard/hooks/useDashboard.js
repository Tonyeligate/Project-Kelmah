...
export const useDashboard = () => {
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            const handleNewJob = (job) => {
                dispatch(setDashboardData(prev => ({ 
                    ...prev, 
                    quickActions: [
    ...
    const loadRecentActivity = useCallback(async (reset = false) => {
        try {
            ...
            if (reset) {
                ...
                dispatch(setDashboardData(prev => ({
                    ...prev,
                    recentActivity: [...(prev.recentActivity || []), ...response.activities]
                })));
    ...
    const loadStatistics = useCallback(async (timeframe = 'week') => {
        try {
