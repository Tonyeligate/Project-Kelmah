...
import { Box, Typography, List, ListItem, ListItemIcon, ListItemText, Button, Divider, Paper, Chip, Stack, Badge, keyframes } from '@mui/material';
...
const NearbyJobs = () => {
  ...
  return (
    ...
    >
      {/* Filter Chips */}
      <Box sx={{ px: 2, py: 1, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
        <FilterListIcon sx={{ color: 'text.secondary', mr: 1 }} fontSize="small" />
        <Chip 
          label="All Jobs" 
          size="small"
      ...
      <Paper 
        ...
        sx={{ 
            ...
            p: 2,
            borderRadius: '8px',
            backgroundImage: 'url(https://images.pexels.com/photos/2242633/pexels-photo-2242633.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1)',
      ...
      >
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          {filteredJobs.length === 0 ? (
            <Typography color="white" textAlign="center" py={3}>No jobs matching the selected filter.</Typography>
          ) : (
                ...
                <React.Fragment key={index}>
                  <ListItem 
                    sx={{ 
                      ...
                      position: 'relative',
                      animation: job.type === 'urgent' ? `${pulseAnimation} 2s infinite` : 'none',
                      bgcolor: job.type === 'urgent' ? 'rgba(255, 0, 0, 0.15)' : 'transparent',
                      borderRadius: '4px',
                      px: 1
                    }}
                  >
                    ...
                    <Box sx={{ 
                      ...
                      >
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <Box sx={{ 
                            mr: 1.5, 
                            color: job.type === 'urgent' ? '#ff6b6b' : '#FFD700',
                            display: 'flex'
                          }}>
                            {job.tradeIcon}
                          </Box>
                    ...
                    <Box sx={{ 
                      ...
                      <Typography variant="body2" color="rgba(255, 255, 255, 0.7)" sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {index < filteredJobs.length - 1 && <Divider component="li" sx={{ borderColor: 'rgba(255, 255, 255, 0.2)' }} />}
                </React.Fragment>
              ))}
            </List>
          )}
