...
import { Container, Paper, Typography, Box, Button, Divider, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{
        ...
        <Typography variant="h4" fontWeight="bold" sx={{ mb: 2, color: 'secondary.main' }}>
          {escrow.title}
        </Typography>
        <Divider sx={{ mb: 2 }} />
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle1" fontWeight="bold">Other Party:</Typography>
          <Typography variant="body1">{escrow.otherParty}</Typography>
        </Box>
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle1" fontWeight="bold">Amount:</Typography>
          <Typography variant="body1">{currencyFormatter.format(escrow.amount)}</Typography>
        </Box>
        <Box sx={{ mb: 4 }}>
      ...
      >
        <DialogTitle sx={{ color: 'secondary.main' }}>Release Funds to {escrow.otherParty}</DialogTitle>
        <DialogContent>
          <Typography gutterBottom>Amount: {currencyFormatter.format(escrow.amount)}</Typography>
          <FormControl fullWidth margin="dense">
            ...
            >
              {paymentMethods.map(pm => (
                <MenuItem key={pm.id} value={pm.id}>{pm.name || pm.type}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </DialogContent>
