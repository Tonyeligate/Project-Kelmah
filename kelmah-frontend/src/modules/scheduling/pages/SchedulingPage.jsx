...
const getStatusChip = (status) => {
  const statusConfig = {
    ...
    pending: { icon: <HourglassEmptyIcon />, label: 'Pending', color: 'warning' },
...
const AppointmentCard = ({ appointment, onEdit, onDelete }) => {
  ...
  return (
    ...
    <Grid container spacing={2} alignItems="center">
      ...
      <Grid item xs={9} sm={7}>
        <Typography variant="h6" fontWeight="bold">{appointment.jobTitle}</Typography>
        ...
        <Typography variant="body2" color="text.secondary">
              {isVirtual ? 'Virtual Meeting' : appointment.location || 'In-person'}
        </Typography>
          </Box>
      </Grid>
      <Grid item xs={12} sm={3} textAlign={{ xs: 'left', sm: 'right' }} sx={{mt: {xs: 1, sm: 0}}}>
  ...
  <Paper elevation={2} sx={{ borderRadius: 2, p: 2, mb: 2 }}>
    <Grid container spacing={2}>
      ...
      <Grid item xs={12} sm={3} textAlign={{ xs: 'left', sm: 'right' }}>
        <Skeleton variant="rounded" width={80} height={24} sx={{ ml: { xs: 0, sm: 'auto' } }} />
      </Grid>
    </Grid>
  </Paper>
);
...
const SchedulingPage = () => {
  ...
  const appointmentDays = appointments.map(a => format(new Date(a.date), 'yyyy-MM-dd'));
  ...
  const handleSubmit = async () => {
    try {
      if (dialogMode === 'create') {
        ...
        await schedulingService.updateAppointment(currentAppointment.id, formData);
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 4 }}>
        ...
        <CalendarTodayIcon sx={{ fontSize: 36, mr: 1.5, color: 'primary.main' }} />
      ...
      {viewMode === 'calendar' ? (
        ...
        <Paper sx={{p: 4, textAlign: 'center', backgroundColor: 'background.default'}} elevation={0}>
        ...
        <Paper sx={{p: 4, textAlign: 'center', backgroundColor: 'background.default'}} elevation={0}>
              <Typography>No upcoming appointments in the next 7 days.</Typography>
        </Paper>
          )}
        </Box>
      )}
