...
import { fetchHirerJobs, updateJobStatus, deleteJob, selectHirerLoading, selectHirerError } from '../services/hirerSlice';
...
const JobManagementPage = () => {
  ...
  const jobs = useMemo(() => Object.values(jobsByStatus).flat(), [jobsByStatus]);
  ...
  useEffect(() => {
    ['active', 'draft', 'completed', 'cancelled', 'paused'].forEach(status => {
          ...
          <Tab 
            label={<Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              {jobs.filter(job => job.status === 'expired').length > 0 && (
                <Chip
                  label={jobs.filter(job => job.status === 'expired').length}
                  size="small"
                  color="error"
                  sx={{ ml: 1, height: 20, fontSize: '0.75rem' }}
                />
      ...
      >
        ...
        {selectedJob?.status !== 'active' && (
          <MenuItem onClick={() => handleStatusChange('active')}>
            <ActiveIcon fontSize="small" sx={{ mr: 1, color: 'success.main' }} />
            Set as Active
          </MenuItem>
        )}
        ...
        {selectedJob?.status !== 'paused' && (
          <MenuItem onClick={() => handleStatusChange('paused')}>
            <PausedIcon fontSize="small" sx={{ mr: 1, color: 'warning.main' }} />
            Pause Job
          </MenuItem>
        )}
