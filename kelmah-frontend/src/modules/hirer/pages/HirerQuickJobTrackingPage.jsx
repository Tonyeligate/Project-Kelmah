...
const QuoteCard = ({ quote, onAccept, actionLoading, acceptedQuoteId }) => {
  ...
  const isAccepted =
    String(acceptedQuoteId || '') === String(quote?._id || quote?.id || '');
      ...
      <CardContent>
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {Array.isArray(quote?.worker?.skills) &&
                quote.worker.skills.length > 0
        ...
        <Grid container spacing={2} sx={{ mt: 0.5 }}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <Typography variant="body2">
              {quote?.estimatedDuration || 'half_day'}
            </Typography>
          </Grid>
        </Grid>
        ...
        >
          <Button
            ...
            disabled={
              actionLoading || isAccepted || quote?.status !== 'pending'
            }
...
const HirerQuickJobTrackingPage = () => {
  ...
  const fetchJob = useCallback(async () => {
    try {
      ...
      if (!result?.success) {
        throw new Error(
          result?.error?.message || 'Unable to load quick-hire request',
        );
      }
        ...
        if (!result?.success) {
          throw new Error(
            result?.error?.message ||
              result?.message ||
              'Unable to verify payment callback',
          );
        }
  ...
  const handleAcceptQuote = async (quote) => {
    ...
    try {
      ...
      if (!result?.success) {
        throw new Error(
          result?.error?.message || 'Unable to accept this quote',
        );
      }
      enqueueSnackbar(
        'Quote accepted. You can now secure the worker with payment.',
        {
          variant: 'success',
        },
      );
  ...
  const handleStartPayment = async () => {
    try {
      ...
      if (!result?.success) {
        throw new Error(
          result?.error?.message || 'Unable to initialize payment',
        );
      }
      ...
      if (redirectUrl) {
        ...
        if (!safeRedirectUrl) {
          throw new Error(
            'Received an invalid payment redirect URL. Please try again.',
          );
        }
  ...
  const handleApproveWork = async () => {
    try {
      ...
      enqueueSnackbar(
        result?.message || 'Work approved and payment released.',
        {
          variant: 'success',
        },
      );
  ...
  const handleCancelJob = async () => {
    try {
      ...
      if (!result?.success) {
        throw new Error(
          result?.error?.message || 'Unable to cancel this request',
        );
      }
      ...
      setError(
        cancelError?.message || 'Unable to cancel this quick-hire request',
      );
    } finally {
      setActionLoading(false);
    }
  ...
  if (error && !job) {
    return (
      <Container maxWidth="sm" sx={{ py: 8 }}>
        ...
        <Button
          variant="contained"
          onClick={() => navigate('/hirer/quick-hire')}
        >
          Back to quick-hire
        </Button>
      </Container>
          ...
          <Grid item xs={12} md={8}>
            <Card sx={{ borderRadius: 3, mb: 3 }}>
              <CardContent>
                ...
                <Typography sx={{ mt: 2.5, mb: 2 }}>
                  {job?.description}
                </Typography>

                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  {job?.location?.address && (
                    <Chip label={job.location.address} variant="outlined" />
                  )}
                  {job?.location?.region && (
                    <Chip label={job.location.region} variant="outlined" />
                  )}
                  {job?.escrow?.amount ? (
                    <Chip
                      label={`Escrow ${formatCurrency(Number(job.escrow.amount || 0))}`}
                      variant="outlined"
                    />
                  ) : null}
                </Stack>
                ...
                <Card sx={{ borderRadius: 3, mb: 3 }}>
                  <CardContent>
                    ...
                    <Typography color="text.secondary" sx={{ mb: 2.5 }}>
                      Compare who is available, how much they charge, and how
                      fast they can get to you.
                    </Typography>
                    <Stack spacing={2}>
                      {quotes.map((quote) => (
                        <QuoteCard
                  ...
                  >
                    <Stack direction="row" spacing={1.5} alignItems="center">
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {Array.isArray(acceptedWorker.skills) &&
                          acceptedWorker.skills.length > 0
                    ...
                    >
                      ...
                      >
                        {formatCurrency(
                          Number(job?.acceptedQuote?.amount || 0),
                        )}
                      </Typography>
                ...
                <CardContent>
                  ...
                  {canPay ? (
                    ...
                    <Stack spacing={2}>
                      ...
                      >
                        The worker marked this request as complete. Review the
                        photos and approve the payout if the job is done.
                      </Alert>
                      <Button
                        variant="contained"
                    ...
                    <Stack spacing={1.5}>
                      <Typography color="text.secondary">
                        {job?.status === 'pending' &&
                          'Wait for workers to send quotes, then accept the best fit for your request.'}
                        {job?.status === 'quoted' &&
                          'Quotes are available now. Accept one to move this request forward.'}
                        {job?.status === 'funded' &&
                          'Payment is secured. Your worker can now start travelling to your location.'}
                        {job?.status === 'worker_on_way' &&
                          'Your worker is travelling now. Keep your phone available in case they need directions.'}
                        {job?.status === 'worker_arrived' &&
                          'The worker has arrived on site and can begin work.'}
                        {job?.status === 'in_progress' &&
                          'Work is currently in progress. You can raise a dispute if something goes wrong.'}
                        {job?.status === 'approved' &&
                          'This quick-hire request is complete and the payout has been released.'}
                        {job?.status === 'cancelled' &&
                          'This quick-hire request has been cancelled.'}
                        {job?.status === 'disputed' &&
                          'A dispute is open on this request. Keep evidence ready for review.'}
                      </Typography>
                ...
                <CardContent>
                  ...
                  <Stack spacing={1.5}>
                    ...
                    <Box>
                      ...
                      <Typography>
                        {formatCurrency(Number(job?.escrow?.platformFee || 0))}
                      </Typography>
                    </Box>
                    {job?.dispute?.status && (
                      <>
                        ...
                        <Box>
                          ...
                          <Typography sx={{ textTransform: 'capitalize' }}>
                            {job.dispute.status.replace(/_/g, ' ')}
                          </Typography>
                        </Box>
                      </>
                    )}
        ...
        >
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ pt: 1 }}>
              <Typography>
                Rate the completed work before the payout is released.
              </Typography>
              <Rating
                value={rating}
