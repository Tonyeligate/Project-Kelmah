...
const hirerAnalyticsService = {
  ...
  getWorkerPerformance: async (hirerId) => {
    try {
      ...
      return normalizeListContainer(unwrapAnalyticsPayload(response), 'workers', []) || {
        workers: [],
      };
    } catch {
  ...
  getHiringEfficiencyMetrics: async (hirerId) => {
    try {
      ...
      return payload && typeof payload === 'object'
        ...
        : analyticsUnavailable({ averageTimeToHire: 0, avgCostPerHire: 0, metrics: [] });
    } catch {
      return analyticsUnavailable({ averageTimeToHire: 0, avgCostPerHire: 0, metrics: [] });
    }
  },
};
