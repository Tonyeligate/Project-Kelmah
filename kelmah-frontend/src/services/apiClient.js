...
const isRefreshRequest = (request) =>
  ...
  /\/auth\/(refresh|refresh-token)/.test(request.url);
...
const isPublicAuthRequest = (request) =>
  typeof request?.url === 'string' &&
  /\/auth\/(login|register|forgot-password|reset-password|resend-verification-email|verify-email|oauth\/exchange)/.test(
    request.url,
  );
...
const enqueueUnauthorizedRequest = (request) =>
  new Promise((resolve, reject) => {
    ...
    if (
      pendingUnauthorizedRequests.length >= MAX_PENDING_UNAUTHORIZED_REQUESTS
    ) {
...
const initializeCrossTabAuthSync = () => {
  if (authSyncListenersInitialized || typeof window === 'undefined') {
    return;
  }
  ...
  async (error) => {
    ...
    if (
      ...
      const hasRefreshCapability =
        AUTH_CONFIG.httpOnlyCookieAuth ||
        Boolean(secureStorage.getRefreshToken());
    ...
    if (!error.normalizedError) {
      ...
      error.friendlyMessage =
        error.friendlyMessage || error.normalizedError.userMessage;
      error.retryable =
      ...
      if (
        ...
        Number.isFinite(error.normalizedError.retryAfterMs)
      ) {
