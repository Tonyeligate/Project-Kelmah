...
class AIMatchingService {
  constructor() {
    ...
    this.ghanaCertifications = {
      ...
      'Institute of Electrical Engineers': { weight: 0.9, trades: ['electrical'] },
      'Ghana Standards Authority': { weight: 0.8, trades: ['all'] },
      'National Vocational Training Institute': { weight: 0.7, trades: ['all'] },
      'Council for Technical and Vocational Education': { weight: 0.7, trades: ['all'] },
    ...
    this.ghanaLanguages = {
      ...
      'Hausa': { regions: ['Northern', 'Upper East', 'Upper West'], speakers: 0.04 }
    };
  }
  ...
  async findBestMatches(jobRequest, availableWorkers, options = {}) {
    try {
      console.log(`🧠 AI Matching: Finding matches for ${jobRequest.title} in ${jobRequest.location}`);
      
      const matches = [];
      
      for (const worker of availableWorkers) {
        const matchScore = await this.calculateMatchScore(jobRequest, worker, options);
        ...
        if (matchScore.totalScore >= (options.minimumScore || 0.4)) {
          matches.push({
            ...
            recommendations: this.generateRecommendations(jobRequest, worker, matchScore)
          });
        }
      }
  ...
  calculateLocationScore(job, worker) {
    ...
    if (distance <= 5) distanceScore = 1.0;      // Within 5km
    else if (distance <= 15) distanceScore = 0.8; // Within 15km
    else if (distance <= 30) distanceScore = 0.6; // Within 30km
    else if (distance <= 50) distanceScore = 0.4; // Within 50km
    else distanceScore = 0.2;                     // Beyond 50km

    // Transport accessibility bonus
    const transportScore = this.calculateTransportScore(jobLocation, workerLocation);
    
    // Regional preference (same region bonus)
    const regionalScore = jobLocation.region === workerLocation.region ? 1.0 : 0.7;
  ...
  calculateSkillsScore(job, worker) {
    ...
    const certificationScore = this.calculateCertificationScore(job.category, worker.certifications || []);
    
    // Experience level matching
    const experienceScore = this.calculateExperienceScore(job.experienceLevel, worker.experience);
    
    // Specialization bonus
    const specializationScore = this.calculateSpecializationScore(job.category, worker.specializations || []);
  ...
  calculatePricingScore(job, worker) {
    ...
    const paymentScore = this.calculatePaymentCompatibility(job.paymentTerms, worker.acceptedPayments);
  ...
  calculateReliabilityScore(worker) {
    ...
    if (responseTime <= 1) responseScore = 1.0;      // Within 1 hour
    else if (responseTime <= 6) responseScore = 0.8; // Within 6 hours
    else if (responseTime <= 24) responseScore = 0.6; // Within 24 hours
  ...
  calculateCulturalScore(job, worker) {
    ...
    const culturalFitScore = this.calculateCulturalFit(jobLocation, workerLocation, worker);
  ...
  parseLocation(locationString) {
    ...
    for (const [regionName, regionData] of Object.entries(this.ghanaRegions)) {
      if (parts.some(part => regionData.major_cities.includes(part))) {
        region = regionName;
        city = parts.find(part => regionData.major_cities.includes(part)) || city;
        break;
      }
    }
    
    return { region, city, coordinates: this.ghanaRegions[region]?.coordinates };
  }
  ...
  calculateDistance(loc1, loc2) {
    ...
    const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
              Math.cos(this.toRad(loc1.coordinates.lat)) * Math.cos(this.toRad(loc2.coordinates.lat)) *
  ...
  calculateTransportScore(jobLoc, workerLoc) {
    ...
    const jobTransport = this.ghanaRegions[jobLoc.region]?.transport_hubs.includes(jobLoc.city);
    const workerTransport = this.ghanaRegions[workerLoc.region]?.transport_hubs.includes(workerLoc.city);
  ...
  calculateCertificationScore(jobCategory, workerCertifications) {
    ...
    for (const [certName, certData] of Object.entries(this.ghanaCertifications)) {
      if (certData.trades.includes(jobCategory) || certData.trades.includes('all')) {
  ...
  calculateLanguageCompatibility(jobRegion, workerLanguages) {
    ...
    for (const [language, langData] of Object.entries(this.ghanaLanguages)) {
      if (workerLanguages.includes(language)) {
        if (langData.regions.includes(jobRegion) || langData.regions.includes('all')) {
  ...
  applyGhanaBoosting(matches, jobRequest) {
    return matches.map(match => {
      ...
      match.matchScore.totalScore = Math.min(match.matchScore.totalScore + boost, 1.0);
  ...
  generateMatchReasoning(matchScore) {
    ...
    return reasons.length > 0 ? reasons : ['Good overall match for your project needs'];
  }
  ...
  generateRecommendations(job, worker, matchScore) {
    ...
    if (matchScore.breakdown.skills.rawScore < 0.8) {
      recommendations.push('Verify specific skill requirements during interview');
    }
  ...
  calculatePaymentCompatibility(jobPayment, workerPayments) {
    ...
    const overlap = jobPayment.filter(method => workerPayments.includes(method));
