...
class EventConsumer {
  constructor() {
    ...
    this.queueName = "payment-service-queue";
  ...
  async connect() {
    try {
      ...
      console.error(
        "Failed to connect to RabbitMQ for event consumption:",
        error,
      );
      setTimeout(() => this.connect(), this.reconnectInterval);
    }
  ...
  async startConsuming() {
    try {
      ...
      const bindingPatterns = [
        ...
        "contract.completed", // When a contract is completed
        "user.wallet.updated", // When a user's wallet is updated
        "auth.user.created", // When a new user is created
      ];
  ...
  async close() {
    try {
      ...
      console.error(
        "Error closing RabbitMQ connection for event consumption:",
        error,
      );
    }
  }
