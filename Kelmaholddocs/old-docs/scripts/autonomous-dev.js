...
function analyzeFilePurpose(filePath) {
  try {
    ...
    switch (ext) {
      ...
      default:
        return { type: 'Unknown', description: 'File type not recognized' };
    }
  } catch (error) {
    return { type: 'Error', description: `Could not analyze file: ${error.message}` };
  }
}
...
function analyzeJavaScriptFile(content) {
  ...
  if (es6Exports) {
    purpose.exports.push(...es6Exports.map(match => match.match(/(\w+)$/)[1]));
  }

  // Extract imports
  const requireMatches = content.match(/require\(['"]([^'"]+)['"]\)/g);
  if (requireMatches) {
    purpose.imports = requireMatches.map(match => 
      match.match(/['"]([^'"]+)['"]/)[1]
    );
  }
  ...
  if (importMatches) {
    purpose.imports.push(...importMatches.map(match => 
      match.match(/['"]([^'"]+)['"]/)[1]
    ));
  }
  ...
  if (arrowFunctions) {
    purpose.functions.push(...arrowFunctions.map(match => 
  ...
  if (componentMatches && content.includes('jsx') || content.includes('React')) {
    purpose.components = componentMatches.map(match => 
      match.match(/const\s+(\w+)/)[1]
    ).filter(name => /^[A-Z]/.test(name)); // React components start with uppercase
  }

  // Generate description
  purpose.description = `This file contains ${purpose.functions.length} functions, ` +
    `${purpose.components.length} components, exports ${purpose.exports.join(', ') || 'none'}, ` +
    `and imports from ${purpose.imports.length} modules.`;
...
function analyzeJsonFile(content) {
  try {
    ...
    return {
      ...
      description: `JSON file with ${Object.keys(json).length} top-level properties`
    };
  } catch (error) {
    return {
      type: 'JSON',
...
function analyzeTextFile(content) {
  ...
  return {
    ...
    lines,
    words,
    description: `Text file with ${lines} lines and approximately ${words} words`
  };
}
