  ...
  for (let i = 0; i < itemsPerCategory; i += 1) {
    ...
    switch (category.title) {
      case 'Responsive Layout & Breakpoints':
        action = `${verb} the responsive behavior in \\`${fileSample}\\` to ensure it adapts cleanly from small phones to large desktop views, avoiding horizontal scroll and clipped content.`;
        break;
      case 'Theme & Visual Consistency':
        action = `${verb} the color, typography, and spacing usage in \\`${fileSample}\\` to match the Kelmah theme and ensure WCAG AA contrast, consistent spacing, and predictable UI hierarchy.`;
        break;
      case 'Component Consistency & Reuse':
        action = `${verb} the repeated UI pattern in \\`${fileSample}\\` and factor it into a shared component or hook to reduce duplication and improve maintainability.`;
        break;
      case 'Accessibility & Keyboard Navigation':
        action = `${verb} \\`${fileSample}\\` for keyboard accessibility (focus order, tab stops, ARIA attributes) so users can navigate without a mouse and screen readers announce meaningful labels.`;
        break;
      case 'Performance & Rendering Optimization':
        action = `${verb} rendering performance in \\`${fileSample}\\` by eliminating unnecessary re-renders, memoizing expensive calculations, and using virtualization for long lists.`;
        break;
      case 'Data & API Patterns':
        action = `${verb} the data-fetching approach in \\`${fileSample}\\` to handle errors gracefully, cancel stale requests, avoid redundant calls, and keep UI state in sync with the backend.`;
        break;
      case 'Logging, Telemetry & Debugging':
        action = `${verb} logging/telemetry around \\`${fileSample}\\` to ensure production builds do not emit dev-only console logs and that key user journeys are captured in analytics.`;
        break;
      case 'Developer Experience & Documentation':
        action = `${verb} the contributor documentation for \\`${fileSample}\\` so new developers can understand architecture, run the project locally, and extend the codebase confidently.`;
        break;
      case 'Mobile Gestures & Touch UX':
        action = `${verb} touch/gesture interactions in \\`${fileSample}\\` to ensure swipe, tap, long-press, and drag gestures behave consistently across Android and iOS devices.`;
        break;
      case 'Localization & Internationalization Readiness':
        action = `${verb} text output in \\`${fileSample}\\` to make it extractable for translation, avoid hardcoded strings, and support locale-specific formatting (dates, numbers, currency).`;
        break;
      default:
        action = `${verb} \\`${fileSample}\\` for general improvement.`;
    }
