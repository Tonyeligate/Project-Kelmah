...
const ServiceClient = {
  ...
  user: {
    ...
    getUsersByIds: async (userIds) => {
      try {
        const response = await userServiceClient.post('/users/batch', { userIds });
        return response.data;
      } catch (error) {
        throw new Error(`User service error: ${error.message}`);
      }
  ...
  payment: {
    createInvoice: async (invoiceData) => {
      try {
        const response = await paymentServiceClient.post('/payments/invoices', invoiceData);
        return response.data;
      } catch (error) {
    ...
    checkPaymentStatus: async (paymentId) => {
      try {
        const response = await paymentServiceClient.get(`/payments/${paymentId}/status`);
        return response.data;
      } catch (error) {
        throw new Error(`Payment service error: ${error.message}`);
      }
  ...
  job: {
    ...
    updateJobStatus: async (jobId, status) => {
      try {
        const response = await jobServiceClient.patch(`/jobs/${jobId}/status`, { status });
        return response.data;
      } catch (error) {
        throw new Error(`Job service error: ${error.message}`);
      }
