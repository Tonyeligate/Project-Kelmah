...
const serviceUrls = {
  ...
  REVIEW: process.env.REVIEW_SERVICE_URL || 'http://localhost:3006/api'
};
...
const createServiceClient = (baseURL) => {
  ...
  client.interceptors.response.use(
    response => response,
    error => {
      console.error(`[Service Error] ${error.config?.method?.toUpperCase()} ${error.config?.url}: ${error.message}`);
...
const ServiceClient = {
  ...
  auth: {
    validateToken: async (token) => {
      try {
        const response = await authServiceClient.post('/auth/validate', { token });
    ...
    getUserPermissions: async (userId) => {
      try {
        const response = await authServiceClient.get(`/auth/permissions/${userId}`);
        return response.data;
      } catch (error) {
        throw new Error(`Auth service error: ${error.message}`);
      }
  ...
  user: {
    ...
    getUsersBySkills: async (skills) => {
      try {
        const response = await userServiceClient.post('/users/search/skills', { skills });
        return response.data;
      } catch (error) {
        throw new Error(`User service error: ${error.message}`);
      }
  ...
  messaging: {
    sendJobNotification: async (userId, jobData) => {
      try {
        const response = await messagingServiceClient.post('/notifications/job', {
          userId,
          jobData
        });
        return response.data;
    ...
    createJobConversation: async (jobId, participantIds) => {
      try {
        const response = await messagingServiceClient.post('/conversations/job', {
          jobId,
  ...
  payment: {
    createPaymentRequest: async (paymentData) => {
      try {
        const response = await paymentServiceClient.post('/payments/requests', paymentData);
        return response.data;
      } catch (error) {
