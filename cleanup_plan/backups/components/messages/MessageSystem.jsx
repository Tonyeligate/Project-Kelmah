...
function MessageSystem() {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            await api.post(`/api/messages/${selectedConversation.id}`, {
                content: newMessage
            });
            setNewMessage('');
            fetchMessages(selectedConversation.id);
                    ...
                    <List>
                        {conversations.map((conversation) => (
                            ...
                            >
                                ...
                                <ListItemText
                                    primary={conversation.worker_id === user.userId ? 
                                ...
                                {loading ? (
                                    ...
                                    messages.map((message) => (
                                        <Box
                                            ...
                                            sx={{
                                                display: 'flex',
                                                justifyContent: message.sender_id === user.userId ? 
                                                    'flex-end' : 'flex-start',
                                                mb: 2
                                            }}
                                        >
                                            <Box
                                                sx={{
                                                    maxWidth: '70%',
                                                    p: 2,
                                                    bgcolor: message.sender_id === user.userId ? 
                                                        'primary.main' : 'grey.100',
                                                    color: message.sender_id === user.userId ? 
                                                        'white' : 'text.primary',
                                                    borderRadius: 2
                                                }}
                                            >
