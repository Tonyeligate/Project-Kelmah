/**
 * Vercel Serverless Function — Create Conversation Bridge (CJS)
 *
 * Bypasses the Render API Gateway proxy body-stream hang for POST /conversations.
...
function forwardRequest(url, method, headers, body) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.setTimeout(55000, () => req.destroy(new Error('Upstream timeout after 55s')));
